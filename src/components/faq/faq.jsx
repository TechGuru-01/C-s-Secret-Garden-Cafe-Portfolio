...
import { HelpCircle, ChevronDown, AlertCircle, ChevronUp } from "lucide-react";
import {
  FernSilhouette,
...
export default function FAQ() {
  ...
  // Number of items to show initially on mobile
  const MOBILE_LIMIT = 3;

  return (
    <section
      id="faq"
      className="relative py-20 px-4 sm:px-6 lg:px-8 bg-[#FFFDF8] overflow-hidden border-b border-sunflower/10"
    >
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        >
          ...
          <p className="text-sm text-charcoal/70 mt-3 leading-relaxed">
            Everything you need to know about dining, reservations, amenities,
            and location before entering the garden.
          </p>
        </motion.div>
        ...
        <div className="space-y-4 text-left">
          {faqs.map((faq, idx) => {
            ...
            const isHiddenOnMobile = idx >= MOBILE_LIMIT && !isMobileExpanded;
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`p-1 rounded-full bg-cream text-sunflower transition-transform duration-300 ${
                      isOpen
                        ? "rotate-180 bg-sunflower text-white font-bold"
                        : ""
                    }`}
                  >
        ...
        {faqs.length > MOBILE_LIMIT && (
          <div className="sm:hidden mt-6 flex justify-center">
            <button
              onClick={() => setIsMobileExpanded(!isMobileExpanded)}
              className="flex items-center gap-1.5 px-5 py-2.5 rounded-full border border-sunflower/30 bg-white text-sunflower font-bold text-xs shadow-sm hover:bg-cream/20 transition-all cursor-pointer"
            >
              <span>
                {isMobileExpanded ? "See Less" : "See More Questions"}
              </span>
              {isMobileExpanded ? (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-charcoal/60 mt-0.5">
                Our team is always happy to assist with special arrangements or
                events.
              </p>
            </div>
          </div>
